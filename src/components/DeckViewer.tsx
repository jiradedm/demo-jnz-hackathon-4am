...
/** กรอบสไลด์แบบ PowerPoint — แถบบน + หมายเลขมุมล่างขวา + มุมตกแต่ง */
function SlideFrame({
  slide,
  slideIndex,
  total,
...
function SlideContent({
  ...
  const frame = (node: React.ReactNode, noFrame?: boolean) => (
    <SlideFrame slide={slide} slideIndex={slideIndex} total={total} noFrame={noFrame}>
      {node}
    </SlideFrame>
  )
      ...
      <div className="flex-1 flex min-h-0">
        ...
        <div className="flex-1 flex flex-col min-w-0">
          <div className="flex-1 flex items-center justify-center p-4 min-h-0">
            <Card className={cn('w-full aspect-video overflow-hidden shadow-xl border-2 border-slate-200/80 dark:border-slate-700', 'max-w-6xl 2xl:max-w-7xl')}>
              <Swiper
